...
describe('Multipart save', () => {
  ...
  it('Multipart saving with groups', () => {
    ...
    cy.get(appFrontend.group.showGroupToContinue).find('input').uncheck();
    ...
    cy.get(appFrontend.group.row(0).nestedGroup.row(1).nestedDynamics).check();
    expectSave(`${groupKey}[0].${subGroupKey}[1].extraOptionsToggle`, 'Ja', null);

    cy.get(appFrontend.group.row(0).nestedGroup.row(1).nestedOptions[2]).check();
    expectSave(`${groupKey}[0].${subGroupKey}[1].extraOptions`, 'o111', null);

    cy.get(appFrontend.group.row(0).nestedGroup.row(1).nestedOptions[1]).check();
    expectSave(`${groupKey}[0].${subGroupKey}[1].extraOptions`, ['o111', 'o1'], ['o111']);

    cy.get(appFrontend.group.row(0).nestedGroup.row(1).nestedOptions[0]).check();
    expectSave(`${groupKey}[0].${subGroupKey}[1].extraOptions`, ['o111', 'o1', 'o11'], ['o111', 'o1']);

    cy.get(appFrontend.group.row(0).nestedGroup.row(1).nestedOptions[2]).uncheck();
    expectSave(`${groupKey}[0].${subGroupKey}[1].extraOptions`, ['o1', 'o11'], ['o111', 'o1', 'o11']);

    cy.get(appFrontend.group.row(0).nestedGroup.row(1).nestedOptions[1]).uncheck();
