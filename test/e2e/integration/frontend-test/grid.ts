...
describe('Grid component', () => {
  it('should work with basic table functionality', () => {
    ...
    cy.get(appFrontend.grid.showGridWithAll).findByRole('radio', { name: /Nei/ }).click();
    cy.get(appFrontend.grid.gridWithAll).should('not.exist');

    // Dynamics hiding an entire row
    cy.get(appFrontend.grid.hasCreditCard).findByRole('radio', { name: /Nei/ }).click();
    cy.get(appFrontend.grid.grid).find('tr').should('have.length', 4);
    cy.get(appFrontend.grid.hasCreditCard).findByRole('radio', { name: /Ja/ }).click();
    ...
    cy.get(appFrontend.grid.hasCreditCard).find('label:contains("Nei")').click({ force: true });
