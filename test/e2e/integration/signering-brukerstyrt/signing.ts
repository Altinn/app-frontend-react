...
describe('Signing', () => {
  ...
  it('should sign on behalf of specified company or the user itself', () => {
    // Step 1: Log in as the initial user
    cy.startAppInstance(appFrontend.apps.signeringBrukerstyrt, { tenorUser: tenorUsers.snaalDugnad });
    // // Step 2: Fill in the form and specify other valid users as signees
    // Om selskapet
    cy.findByRole('textbox', { name: /navn/i }).type('Testselskap AS');
    cy.findByRole('button', { name: /neste/i }).click();
    ...
    // Stiftere og aksjetegning
    cy.findByRole('button', { name: /legg til person/i }).click();
    cy.findByRole('textbox', { name: /fødselsnummer/i }).type(tenorUsers.snaalDugnad.ssn);
    cy.findByRole('textbox', { name: /navn/i }).type(tenorUsers.snaalDugnad.name.split(' ')[1]);
    cy.findByRole('button', { name: /hent opplysninger/i }).click();
    ...
    cy.findByRole('button', { name: /legg til virksomhet/i }).click();
    cy.findByRole('textbox', { name: /organisasjonsnummer/i }).type(tenorOrgs['Tilbakeholden Upopulær Tiger AS'].orgNr);
    cy.findByRole('button', { name: /hent opplysninger/i }).click();
    cy.findByTestId('group-edit-container').within(() => {
      cy.findByRole('button', { name: /lagre og lukk/i }).click();
    });
    cy.findByRole('button', { name: /legg til virksomhet/i }).click();
    cy.findByRole('textbox', { name: /organisasjonsnummer/i }).type(tenorOrgs['Offisiell Virtuell Tiger AS'].orgNr);
    cy.findByRole('button', { name: /hent opplysninger/i }).click();
    cy.findByTestId('group-edit-container').within(() => {
      cy.findByRole('button', { name: /lagre og lukk/i }).click();
    });
    ...
    cy.findByRole('table', {
      ...
      cy.findByRole('row', {
        name: new RegExp(`${reverseName(tenorUsers.snaalDugnad.name)} venter på signering`, 'i'),
      });
      cy.findByRole('row', {
        name: new RegExp(`${tenorOrgs['Tilbakeholden Upopulær Tiger AS'].name}`, 'i'),
      });
      cy.findByRole('row', {
        name: new RegExp(`${tenorOrgs['Offisiell Virtuell Tiger AS'].name}`, 'i'),
      });
    });
    ...
    cy.findByRole('table', {
      ...
      cy.findByRole('row', { name: new RegExp(reverseName(tenorUsers.snaalDugnad.name), 'i') }).within(() => {
        cy.findByRole('cell', { name: /signert/i }).should('exist');
      });
    });
    ...
    // TODO: Sign on behalf of both organizations
