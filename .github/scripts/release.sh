...
if [[ -z "$AZURE_STORAGE_ACCOUNT_NAME" ]]; then
  ...
  if [[ -d "$AZURE_STORAGE_ACCOUNT_NAME" ]]; then
    ...
    schemas_rsync_opts=( -am --include='*/' --include="component/*" --include="layout/*" --include="text-resources/*"  --exclude='*' )
    set -x
    ...
    AZCOPY_SCHEMAS_OPTS=( --include-regex="^component/.*|^layout/.*|^text-resources/.*" )
