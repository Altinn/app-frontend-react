...
function* updateFocus({
  ...
  try {
    ...
    if (currentComponentId) {
      const layout =
        formLayoutState.layouts[formLayoutState.uiConfig.currentView].data
          .layout;
  ...
  try {
    ...
    if (!runValidations) {
      ...
      if (state.formLayout.uiConfig.returnToView) {
        ...
        yield put(
          FormLayoutActions.updateCurrentViewFulfilled({ newView: newViewId })
        );
      } else if (
        !canFormBeSaved(
          {
...
export function* updateFileUploaderWithTagChosenOptionsSaga({
  ...
  try {
    ...
    const state: IRuntimeState = yield select();
    const currentView = state.formLayout.uiConfig.currentView;
    const component = state.formLayout.layouts[currentView].data.layout.find(
