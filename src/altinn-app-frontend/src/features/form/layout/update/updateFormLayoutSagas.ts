...
const selectFormLayoutState = (state: IRuntimeState): ILayoutState =>
  state.formLayout;
const selectFormData = (state: IRuntimeState): IFormDataState => state.formData;
const selectFormLayouts = (state: IRuntimeState): ILayouts =>
  state.formLayout.layouts;
const selectAttachmentState = (state: IRuntimeState): IAttachmentState =>
  state.attachments;
...
export function* updateFileUploaderWithTagEditIndexSaga({
  payload: { uploader, index, attachmentId = null },
  ...
  try {
    if (attachmentId && index === -1) {
      ...
      const state: IRuntimeState = yield select();
      const chosenOption =
        state.formLayout.uiConfig.fileUploadersWithTag[uploader].chosenOptions[
          attachmentId
        ];
...
export function* updateFileUploaderWithTagChosenOptionsSaga({
  ...
  try {
    ...
    const component = state.formLayout.layouts[currentView].find(
      (component: ILayoutComponent) => component.id === uploader,
