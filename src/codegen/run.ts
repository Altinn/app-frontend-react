...
(async () => {
  ...
  const componentIndex = [
    ...
    '',
    ...sortedKeys.map(
      (key) => `import { getConfig as get${key}Config } from 'src/layout/${key}/config.generated${tsPathSuffix}';`,
    ),
    ...sortedKeys.map(
      (key) =>
        `import type { TypeConfig as ${key}TypeConfig } from 'src/layout/${key}/config.generated${tsPathSuffix}';`,
    ),
    '',
  ...
  promises.push(saveFile(`src/layout/components.generated${tsPathSuffix}.ts`, componentIndex.join('\n')));
  ...
  for (const key of sortedKeys) {
    const tsPathConfig = `src/layout/${key}/config.generated${tsPathSuffix}.ts`;
    const tsPathDef = `src/layout/${key}/config.def.generated${tsPathSuffix}.tsx`;
  ...
  const commonTsPath = `src/layout/common.generated${tsPathSuffix}.ts`;
  promises.push(
    saveTsFile(
      commonTsPath,
