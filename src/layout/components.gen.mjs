...
const lines = [
  ...
  ...Object.keys(components).map((key) => `  ${components[key]}: ${key}Config.def,`),
  `};`,
  '',
  `export type ComponentLayoutTypeMap = {`,
  ...Object.keys(components).map((key) => `  ${components[key]}: (typeof ${key}Config)['types']['layout'];`),
  `};`,
  '',
  `export type ComponentNodeItemMap = {`,
  ...Object.keys(components).map((key) => `  ${components[key]}: (typeof ${key}Config)['types']['nodeItem'];`),
  `};`,
  '',
  `export type ComponentNodeObjMap = {`,
  ...Object.keys(components).map((key) => `  ${components[key]}: (typeof ${key}Config)['types']['nodeObj'];`),
  `};`,
  '',
];
