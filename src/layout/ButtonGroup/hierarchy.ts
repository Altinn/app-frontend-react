...
const ButtonComponentTypes: ComponentTypes[] = ['Button', 'NavigationButtons', 'InstantiationButton', 'PrintButton'];
...
export class ButtonGroupHierarchyGenerator extends ComponentHierarchyGenerator<'ButtonGroup'> {
  private isButton(childId: string, outputWarning = true): boolean {
    ...
    if (outputWarning && prototype && !ButtonComponentTypes.includes(prototype.type)) {
      console.warn(
    ...
    return typeof prototype !== 'undefined' && ButtonComponentTypes.includes(prototype.type);
  }
