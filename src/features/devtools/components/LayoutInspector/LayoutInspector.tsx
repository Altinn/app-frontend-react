  ...
  function handleSave() {
    if (selectedComponent) {
      try {
        ...
        if (currentView) {
          window.queryClient.setQueryData<LayoutContextValue>(['formLayouts', currentLayoutSetId], (queryData) =>
            queryData?.layouts?.[currentView]
              ? {
                  ...queryData,
                  layouts: {
                    ...queryData?.layouts,
                    [currentView]: queryData.layouts[currentView]!.map((component) =>
