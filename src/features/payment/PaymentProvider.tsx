...
export const PaymentProvider: React.FC<PaymentContextProvider> = ({ children }) => {
  ...
  const {
    mutateAsync: performPaymentMutation,
    error: paymentError,
    isPending: isPerformingPayment,
  } = usePerformPayActionMutation(instanceOwnerPartyId, instanceGuid);
  const { processNext, isPending: isSkippingPayment } = useProcessNext();

  const isLoading = isSkippingPayment || isPerformingPayment;

  const performPayment = useEffectEvent(() => performPaymentMutation());
  const skipPayment = useEffectEvent(() => processNext({ action: 'confirm' }));
