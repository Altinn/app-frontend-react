...
export function useOnPageNavigationValidation() {
  ...
  const callback = useOurEffectEvent(
    async (currentPage: string, config: PageValidation, direction: string): Promise<boolean> => {
      const pageConfig = config.page ?? 'current';
      const masks = config.show;
      const preventedNavigation = config.preventNavigation ?? 'all';
      ...
      if (
        (direction === 'previous' && preventedNavigation === 'forward') ||
        (direction === 'forward' && preventedNavigation === 'previous')
      ) {
        return false;
      }
  ...
  return useCallback(
    async (currentPage: string, config: PageValidation, direction: string) => {
      await validating();
      return callback(currentPage, config, direction);
    },
    [callback, validating],
  );
}
