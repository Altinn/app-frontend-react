 ...
 * This wraps the earlier applyChanges function, and adds some logging and error handling to make sure
 * the new implementation works the same way as the old one, or better.
 */
export function testApplyChanges({ prev, next, changesPatch, backendChangesPatch, applyTo }: Props) {
  ...
  try {
    ...
    if (!deepEqual(applyToTheirs, afterOurApply)) {
      ...
      log({
        message: `Inspect this: applyChanges did not match fast-json-patch when applying patch from backend changes`,
        diffValues,
        prev,
        next,
        currentUuid,
    ...
    window.CypressSaveLog?.();
    log({
      message: `Inspect this: applyChanges failed to apply patch from backend: ${(err as Error).message}`,
      prev,
      next,
      currentUuid,
    });
